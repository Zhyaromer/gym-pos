...
Items: ${items.length}
Total: ${Number(total).toFixed(3)} د.ع
      `;
        ...
        <div className="mb-4">
          ...
          {items?.map(item => (
            <div key={item.product_id} className="flex justify-between py-2 border-b border-dashed border-gray-200 text-xs">
              ...
              <span className="w-2/5 text-center">{item.selling_price.toFixed(3)} د.ع</span>
            </div>
          ))}
        </div>
                    ...
                    <>
                      <div className="bg-blue-50 p-3 rounded-md">
                        ...
                        <div className="flex justify-between mt-1">
                          ...
                          <span>{productInfo.selling_price.toFixed(3)} د.ع</span>
                        </div>
                      </div>
...
const TransactionView = () => {
  ...
  const [mockTransactions, setMockTransactions] = useState({
    'ORD001': {
      ...
      total_amount: 15000,
      final_amount: 13500,
      transaction_date: '2025-05-31T10:30:00',
      items: [
        {
          item_id: 1,
          product_id: 101,
    ...
    'ORD002': {
      ...
      total_amount: 12000,
      final_amount: 11000,
      transaction_date: '2025-05-31T14:15:00',
      items: [
        {
          item_id: 4,
          product_id: 104,
  ...
  const handleSearch = async () => {
    if (!searchQuery.trim()) {
      setError('تکایە ژمارەی داواکاری داخڵ بکە');
      return;
    }
    ...
    try {
      ...
      const foundTransaction = mockTransactions[searchQuery.toUpperCase()];

      if (foundTransaction) {
        setTransaction(foundTransaction);
      } else {
        setError('هیچ داواکاریەک بەم ژمارەیە نەدۆزرایەوە');
        setTransaction(null);
      }
    } catch (err) {
      setError('هەڵەیەک ڕوویدا لە گەڕان');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div dir='rtl' className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="border-t pt-4 space-y-2">
                  ...
                  {transaction.discount_type !== 'none' && (
                    <div className="flex justify-between text-red-600">
                      <span>داشکاندن ({transaction.discount_type === 'percentage' ? `${transaction.discount_value}%` : 'بڕی دیاریکراو'}):</span>
            ...
            <div>
              <Receipt
                ...
                discount={calculateDiscount(transaction)}
        ...
        {/* Sample Order Numbers Help */}
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-6">
          <h3 className="font-bold text-blue-800 mb-2">نموونەی ژمارەی داواکاری بۆ تاقیکردنەوە:</h3>
          <div className="flex flex-wrap gap-2">
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-mono">ORD001</span>
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-mono">ORD002</span>
          </div>
        </div>
      </div>
